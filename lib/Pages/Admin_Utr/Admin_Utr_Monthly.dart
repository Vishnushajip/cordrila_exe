...
class UtrProviderMonthly extends ChangeNotifier {
  int selectedIndex = 0;
...
class AdminUtrMonthly extends StatefulWidget {
  const AdminUtrMonthly({super.key});

  @override
  _AdminUtrMonthlyState createState() => _AdminUtrMonthlyState();
}

class _AdminUtrMonthlyState extends State<AdminUtrMonthly>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<UtrProviderMonthly>(context);
    return Scaffold(
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<UtrProviderMonthly>(context, listen: false);
    return TextButton(
      onPressed: () {
  ...
  Future<void> _downloadMonthlyData(
    ...
    try {
      ...
      await _downloadCSV(
          context, rows, 'Utr_monthly_data_$formattedDateTime');
    } catch (e) {
