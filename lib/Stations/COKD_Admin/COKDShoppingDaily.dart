...
class COKDShoppingProviderDaily extends ChangeNotifier {
  int selectedIndex = 0;
...
class COKDShoppingPageDaily extends StatefulWidget {
  const COKDShoppingPageDaily({super.key});

  @override
  _COKDShoppingPageDailyState createState() => _COKDShoppingPageDailyState();
}

class _COKDShoppingPageDailyState extends State<COKDShoppingPageDaily>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<COKDShoppingProviderDaily>(context);
    return Scaffold(
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const COKDShoppingPageMonthly(),
                  ),
                );
              },
              icon: const Icon(
        ...
        leading: IconButton(
            onPressed: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const COKDHomePage(),
                  ));
            },
            icon: const Icon(
              Icons.arrow_back,
                  ...
                  PageRouteBuilder(
                    pageBuilder: (context, animation, secondaryAnimation) {
                      return const TransitionPage(
                          destination: COKDShoppingPageMonthly());
                    },
                    transitionsBuilder:
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<COKDShoppingProviderDaily>(context, listen: false);
    return TextButton(
      onPressed: () {
