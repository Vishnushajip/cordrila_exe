...
class COKDAdminUtrDaily extends StatefulWidget {
  const COKDAdminUtrDaily({super.key});

  @override
  _COKDAdminUtrDailyState createState() => _COKDAdminUtrDailyState();
}

class _COKDAdminUtrDailyState extends State<COKDAdminUtrDaily>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<COKDUtrProviderDaily>(context);
    return Scaffold(
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const COKDAdminUtrMonthly(),
                  ),
                );
              },
              icon: const Icon(
                  ...
                  PageRouteBuilder(
                    pageBuilder: (context, animation, secondaryAnimation) {
                      return const TransitionPage(
                          destination: COKDAdminUtrMonthly());
                    },
                    transitionsBuilder:
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<COKDUtrProviderDaily>(context, listen: false);
    return TextButton(
      onPressed: () {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .where('Location', whereIn: ['COKD']).snapshots(),
  ...
  Future<void> _downloadDailyData(BuildContext context) async {
    try {
      ...
      final snapshot = await FirebaseFirestore.instance
          ...
          .where('Date', isLessThanOrEqualTo: endOfDay)
          .where('Utr', isNotEqualTo: true)
          .where('Location', whereIn: ['COKD']).get();
  ...
  Future<void> _downloadMonthlyData(
    ...
    try {
      ...
      await _downloadCSV(context, rows, 'Utr_monthly_data_$formattedMonth.csv');
    } catch (e) {
