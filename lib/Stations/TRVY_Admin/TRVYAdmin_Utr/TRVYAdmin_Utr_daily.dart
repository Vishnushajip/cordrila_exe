...
class TRVYUtrProviderDaily extends ChangeNotifier {
  int selectedIndex = 0;
...
class TRVYAdminUtrDaily extends StatefulWidget {
  const TRVYAdminUtrDaily({super.key});

  @override
  _TRVYAdminUtrDailyState createState() => _TRVYAdminUtrDailyState();
}

class _TRVYAdminUtrDailyState extends State<TRVYAdminUtrDaily>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<TRVYUtrProviderDaily>(context);
    return Scaffold(
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const TRVYAdminUtrMonthly(),
                  ),
                );
              },
              icon: const Icon(
                  ...
                  PageRouteBuilder(
                    pageBuilder: (context, animation, secondaryAnimation) {
                      return const TransitionPage(
                          destination: TRVYAdminUtrMonthly());
                    },
                    transitionsBuilder:
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<TRVYUtrProviderDaily>(context, listen: false);
    return TextButton(
      onPressed: () {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .where('Location', whereIn: ['TRVY']).snapshots(),
  ...
  Future<void> _downloadDailyData(BuildContext context) async {
    try {
      ...
      final snapshot = await FirebaseFirestore.instance
          ...
          .where('Date', isLessThanOrEqualTo: endOfDay)
          .where('Utr', isNotEqualTo: true)
          .where('Location', whereIn: ['TRVY']).get();
