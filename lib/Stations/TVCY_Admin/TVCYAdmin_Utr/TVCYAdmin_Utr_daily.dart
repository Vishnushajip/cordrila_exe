...
class TVCYUtrProviderDaily extends ChangeNotifier {
  int selectedIndex = 0;
...
class TVCYAdminUtrDaily extends StatefulWidget {
  const TVCYAdminUtrDaily({super.key});

  @override
  _TVCYAdminUtrDailyState createState() => _TVCYAdminUtrDailyState();
}

class _TVCYAdminUtrDailyState extends State<TVCYAdminUtrDaily>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<TVCYUtrProviderDaily>(context);
    return Scaffold(
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const TVCYAdminUtrMonthly(),
                  ),
                );
              },
              icon: const Icon(
                  ...
                  PageRouteBuilder(
                    pageBuilder: (context, animation, secondaryAnimation) {
                      return const TransitionPage(
                          destination: TVCYAdminUtrMonthly());
                    },
                    transitionsBuilder:
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<TVCYUtrProviderDaily>(context, listen: false);
    return TextButton(
      onPressed: () {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .where('Location', whereIn: ['TVCY']).snapshots(),
  ...
  Future<void> _downloadDailyData(BuildContext context) async {
    try {
      ...
      final snapshot = await FirebaseFirestore.instance
          ...
          .where('Date', isLessThanOrEqualTo: endOfDay)
          .where('Utr', isNotEqualTo: true)
          .where('Location', whereIn: ['TVCY']).get();
