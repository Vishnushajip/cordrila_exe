...
import 'TVCYShoppingMonthly.dart';
import 'TVCY_Home.dart';

class TVCYShoppingProviderDaily extends ChangeNotifier {
  int selectedIndex = 0;
...
class TVCYShoppingPageDaily extends StatefulWidget {
  const TVCYShoppingPageDaily({super.key});

  @override
  _TVCYShoppingPageDailyState createState() => _TVCYShoppingPageDailyState();
}

class _TVCYShoppingPageDailyState extends State<TVCYShoppingPageDaily>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<TVCYShoppingProviderDaily>(context);
    return Scaffold(
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const TVCYShoppingPageMonthly(),
                  ),
                );
              },
              icon: const Icon(
        ...
        leading: IconButton(
            onPressed: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const TVCYHomePage(),
                  ));
            },
            icon: const Icon(
              Icons.arrow_back,
                  ...
                  PageRouteBuilder(
                    pageBuilder: (context, animation, secondaryAnimation) {
                      return const TransitionPage(
                          destination: TVCYShoppingPageMonthly());
                    },
                    transitionsBuilder:
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<TVCYShoppingProviderDaily>(context, listen: false);
    return TextButton(
      onPressed: () {
  ...
  Future<void> _downloadDailyData(BuildContext context) async {
    try {
      ...
      await _downloadCSV(context, rows, 'Shopping_daily_data_$formattedDate');
    } catch (e) {
