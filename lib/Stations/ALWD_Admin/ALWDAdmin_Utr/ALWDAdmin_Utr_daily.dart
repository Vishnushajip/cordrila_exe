...
import 'package:cordrila_exe/Stations/ALWD_Admin/ALWD_Home.dart';
...
class ALWDUtrProviderDaily extends ChangeNotifier {
  int selectedIndex = 0;
...
class ALWDAdminUtrDaily extends StatefulWidget {
  const ALWDAdminUtrDaily({super.key});

  @override
  _ALWDAdminUtrDailyState createState() => _ALWDAdminUtrDailyState();
}

class _ALWDAdminUtrDailyState extends State<ALWDAdminUtrDaily>
  ...
  Widget build(BuildContext context) {
    final filterProvider = Provider.of<ALWDUtrProviderDaily>(context);
    return Scaffold(
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => const ALWDAdminUtrMonthly(),
                  ),
                );
              },
              icon: const Icon(
                  ...
                  PageRouteBuilder(
                    pageBuilder: (context, animation, secondaryAnimation) {
                      return const TransitionPage(
                          destination: ALWDAdminUtrMonthly());
                    },
                    transitionsBuilder:
  ...
  Widget _buildTabButton(BuildContext context, int index, String text) {
    final filterProvider =
        Provider.of<ALWDUtrProviderDaily>(context, listen: false);
    return TextButton(
      onPressed: () {
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .where('Location', whereIn: ['ALWD']).snapshots(),
